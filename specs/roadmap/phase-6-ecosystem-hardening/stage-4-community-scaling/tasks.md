...
> Source stage: `stage-4-community-scaling`

### Better contributor docs

**What it is**
This implements `Better contributor docs` in `stage-4-community-scaling` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Better contributor docs success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Better contributor docs validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Better contributor docs permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Better issue triage` in `stage-4-community-scaling` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Better issue triage success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Better issue triage validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Better issue triage permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Plugin governance` in `stage-4-community-scaling` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Plugin governance success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Plugin governance validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Plugin governance permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Maintainer workflows` in `stage-4-community-scaling` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Maintainer workflows success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Maintainer workflows validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Maintainer workflows permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Contribution automation` in `stage-4-community-scaling` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Contribution automation success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Contribution automation validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Contribution automation permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
