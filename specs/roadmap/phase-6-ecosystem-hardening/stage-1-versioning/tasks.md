...
> Source stage: `stage-1-versioning`

### Runtime version policy

**What it is**
This implements `Runtime version policy` in `stage-1-versioning` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Runtime version policy success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Runtime version policy validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Runtime version policy permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `SDK version policy` in `stage-1-versioning` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `SDK version policy success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `SDK version policy validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `SDK version policy permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Plugin compatibility rules` in `stage-1-versioning` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Plugin compatibility rules success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Plugin compatibility rules validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Plugin compatibility rules permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Deprecation strategy` in `stage-1-versioning` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Deprecation strategy success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Deprecation strategy validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Deprecation strategy permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
