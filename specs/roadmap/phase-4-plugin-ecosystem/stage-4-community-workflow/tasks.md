...
> Source stage: `stage-4-community-workflow`

### Contribution guide

**What it is**
This implements `Contribution guide` in `stage-4-community-workflow` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Contribution guide success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Contribution guide validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Contribution guide permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Security checklist` in `stage-4-community-workflow` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Security checklist success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Security checklist validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Security checklist permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Plugin review process` in `stage-4-community-workflow` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Plugin review process success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Plugin review process validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Plugin review process permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Issue templates` in `stage-4-community-workflow` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Issue templates success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Issue templates validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Issue templates permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
