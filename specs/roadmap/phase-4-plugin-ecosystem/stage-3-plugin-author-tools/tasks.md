...
> Source stage: `stage-3-plugin-author-tools`

### Plugin template generator

**What it is**
This implements `Plugin template generator` in `stage-3-plugin-author-tools` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Plugin template generator success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Plugin template generator validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Plugin template generator permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Plugin test harness` in `stage-3-plugin-author-tools` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Plugin test harness success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Plugin test harness validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Plugin test harness permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Plugin validation tools` in `stage-3-plugin-author-tools` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Plugin validation tools success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Plugin validation tools validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Plugin validation tools permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Plugin docs template` in `stage-3-plugin-author-tools` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Plugin docs template success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Plugin docs template validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Plugin docs template permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
