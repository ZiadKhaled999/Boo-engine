...
> Source stage: `stage-2-fast-iteration`

### Hot reload

**What it is**
This implements `Hot reload` in `stage-2-fast-iteration` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Hot reload success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Hot reload validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Hot reload permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Live reload` in `stage-2-fast-iteration` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Live reload success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Live reload validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Live reload permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Local dev server support` in `stage-2-fast-iteration` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Local dev server support success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Local dev server support validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Local dev server support permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Runtime refresh flow` in `stage-2-fast-iteration` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Runtime refresh flow success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Runtime refresh flow validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Runtime refresh flow permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Better dev logs` in `stage-2-fast-iteration` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Better dev logs success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Better dev logs validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Better dev logs permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
