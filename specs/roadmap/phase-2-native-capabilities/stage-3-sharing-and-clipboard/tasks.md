...
> Source stage: `stage-3-sharing-and-clipboard`

### Share text

**What it is**
This implements `Share text` in `stage-3-sharing-and-clipboard` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Share text success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Share text validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Share text permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Share files` in `stage-3-sharing-and-clipboard` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Share files success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Share files validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Share files permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Copy clipboard` in `stage-3-sharing-and-clipboard` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Copy clipboard success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Copy clipboard validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Copy clipboard permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
### Read clipboard where allowed

**What it is**
This implements `Read clipboard where allowed` in `stage-3-sharing-and-clipboard` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Read clipboard where allowed success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Read clipboard where allowed validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Read clipboard where allowed permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Add JS APIs` in `stage-3-sharing-and-clipboard` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Add JS APIs success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Add JS APIs validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Add JS APIs permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
