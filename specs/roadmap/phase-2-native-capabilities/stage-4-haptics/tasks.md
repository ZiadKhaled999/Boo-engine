...
> Source stage: `stage-4-haptics`

### Short vibration

**What it is**
This implements `Short vibration` in `stage-4-haptics` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Short vibration success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Short vibration validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Short vibration permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Strong vibration` in `stage-4-haptics` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Strong vibration success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Strong vibration validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Strong vibration permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Pattern vibration` in `stage-4-haptics` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**Tests**
1. `Pattern vibration success` — setup valid config + payload; invoke method; expect `ok=true` and expected side-effect.
2. `Pattern vibration validation failure` — send malformed payload; expect `VALIDATION_ERROR` and no side-effect.
3. `Pattern vibration permission denied` — remove permission or deny OS prompt; expect `PERMISSION_DENIED`.
...
**What it is**
This implements `Add JS wrapper` in `stage-4-haptics` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
...
**What it is**
This implements `Add permission checks` in `stage-4-haptics` as a shippable vertical slice, so contributors can deliver one deterministic behavior before moving to the next part.
